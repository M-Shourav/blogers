import React from "react";
import { signIn } from "../../../auth";
import { Field, Input, Checkbox, Label } from "@headlessui/react";
import Link from "next/link";
import { FaCheck } from "react-icons/fa6";
import clsx from "clsx";

export default function LoginPage() {
  return (
    <div className="fixed top-0 left-0 bg-white w-full h-screen">
      <div className="w-full h-full flex items-center justify-center p-6 lg:p-8">
        <div
          className="w-full max-w-md rounded-xl shadow-md border border-black/10 p-4
        space-y-4"
        >
          <form action="#">
            <div className="flex flex-col gap-y-2">
              <Link
                href={"/"}
                className="text-2xl font-semibold text-black capitalize
             hover:text-amber-800 duration-300 ease-in-out mb-5"
              >
                Bloggers
              </Link>
              <h1 className="text-base font-semibold">Welcome back!</h1>
              <p className="text-sm text-gray-700">
                Sign in to your account to continue.
              </p>
            </div>
            <Field className="mt-4 space-y-5">
              <Label className="text-sm/5 font-semibold">Email</Label>
              <Input
                type="email"
                name="email"
                required
                autoFocus
                placeholder="john.doe@gmail.com"
                className="outline-none w-full h-10 px-4 py-2 rounded-lg mt-2 border
                border-transparent ring-1 ring-black/10 focus-visible:border-blue-500"
              />
            </Field>
            <Field className="mt-2 space-y-5">
              <Label className="text-sm/5 font-semibold">Password</Label>
              <Input
                type="password"
                name="password"
                required
                autoFocus
                className="outline-none w-full h-10 px-4 py-2 rounded-lg mt-2 border
                border-transparent ring-1 ring-black/10 focus-visible:border-blue-500"
              />
            </Field>
            <div className="mt-4 flex items-center justify-between text-sm/5 font-semibold">
              <Field className="flex items-center gap-x-2">
                <Checkbox
                  name="remember-me"
                  className={clsx(
                    "group block size-4 rounded border border-transparent shadow ring-1 ring-black/10 focus:outline-none",
                    "data-[checked]:bg-black data-[checked]:ring-black",
                    "data-[focus]:outline-2 data-[focus]:outline-offset-2 data-[focus]:outline-black"
                  )}
                >
                  <FaCheck className="fill-white opacity-0 group-data-[checked]:opacity-100" />
                </Checkbox>
                <Label>remember me</Label>
              </Field>
            </div>
          </form>
          <div>
            <form
              action={async () => {
                "use server";
                await signIn("google");
              }}
            >
              <button
                type="submit"
                className="w-full py-2 flex items-center justify-center gap-x-2 rounded-full bg-black/80 text-white
                text-base font-semibold hover:bg-black duration-300 ease-in-out cursor-pointer"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="currentColor"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  />
                  <path
                    fill="currentColor"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  />
                </svg>
                Signin with Google
              </button>
            </form>
          </div>
        </div>
      </div>
    </div>
  );
}
